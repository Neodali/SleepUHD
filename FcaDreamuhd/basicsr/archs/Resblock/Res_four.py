...
class FEblock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
        ...
        self.conv2 = DepthwiseSeparableConv(out_channels//2,
                                     ...
                                     padding=1)
        self.mergeconv = nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1, padding=0)
        # FCA: Multi-Spectral Channel Attention al posto del blocco FFT+ampconv0
        # Opera su in_channels canali.
        # dct_h/dct_w=28 (= 7×4): deve essere ≤ feature map minima (32×32 nei mid blocks)
        # e multiplo di 7 (richiesto da get_freq_indices). adaptive_avg_pool2d fa pooling DOWN.
    ...
    def forward(self, x):
        ...
        # FCA: Multi-Spectral Channel Attention sostituisce il blocco FFT+ampconv0
        # Input: [N, in_channels, H, W] -> Output: [N, in_channels, H, W] (x * channel_attention)
        h = self.fca(x)
        h = self.norm1(h)
        x1= self.splitconv1(h)
        x11= torch.fft.rfft2(x1, norm='backward')
