...
class PairedImageDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # CenterCrop opzionale — riduce il picco VRAM su GPU limitate (es. T4 Kaggle).
        # Si attiva aggiungendo `center_crop_size: [H, W]` nel dataset config del YAML.
        # Viene applicato prima di padding/random_crop, su entrambe gt e lq.
        if self.opt.get('center_crop_size') is not None:
            crop_h, crop_w = self.opt['center_crop_size']
            def _center_crop(img):
                ih, iw = img.shape[:2]
                top  = max((ih - crop_h) // 2, 0)
                left = max((iw - crop_w) // 2, 0)
                return img[top:top + min(crop_h, ih), left:left + min(crop_w, iw)]
            img_gt = _center_crop(img_gt)
            img_lq = _center_crop(img_lq)
